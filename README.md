...
- **Catalog resolution.** Catalogs come from the client's inline catalogs, from catalogs you register, or from the built-in basic catalog. Unknown catalogs fall back to envelope checks unless you opt into strict mode. The library never fetches over the network. A catalog need not carry `$defs`: the component and function unions are derived from its `components` and `functions` (and a v0.9 `theme`) when absent.
